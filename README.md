...
2. **점수 분포**: 승패·핸디캡(마진)은 두 팀 득점을 독립 푸아송으로 두고 점수
   조합 격자(0~29점)를 더해 계산합니다. 언더오버(총득점)는 두 팀 득점의 양의
   상관(구장·날씨) 때문에 푸아송보다 분산이 크고 오른쪽으로 치우치므로,
   실측 분산(분산/평균 ≈ 2.2)에 맞춘 **음이항 분포**로 계산합니다 — 마진은
   상관이 상쇄되어 푸아송이 잘 맞는다는 것이 백테스트 캘리브레이션으로
   확인되었습니다.
...
알려진 한계: 구장 효과(파크 팩터)와 불펜 상태는 반영하지 않습니다.
